defmodule Cachex.Actions.PutAndGetTest do
  use CachexCase
  ...
  test "adding new values to the cache" do
    ...
    set1 = Cachex.put_and_get(cache1, 1, 1)
    set2 = Cachex.put_and_get(cache1, 2, 2, ttl: 5000)
    set3 = Cachex.put_and_get(cache2, 1, 1)
    set4 = Cachex.put_and_get(cache2, 2, 2, ttl: 5000)

    # ensure all set actions worked
    assert(set1 == { :ok, true, 1 })
    assert(set2 == { :ok, true, 2 })
    assert(set3 == { :ok, true, 1 })
    assert(set4 == { :ok, true, 2 })

    # verify the hooks were updated with the message
    assert_receive({ { :put_and_get, [ 1, 1, [] ] }, ^set1 })
    assert_receive({ { :put_and_get, [ 1, 1, [] ] }, ^set3 })
    assert_receive({ { :put_and_get, [ 2, 2, [ ttl: 5000 ] ] }, ^set2 })
    assert_receive({ { :put_and_get, [ 2, 2, [ ttl: 5000 ] ] }, ^set4 })
  ...
  test "adding new entries to a cache cluster" do
    ...
    { :ok, true, 1 } = Cachex.put_and_get(cache, 1, 1)
    { :ok, true, 2 } = Cachex.put_and_get(cache, 2, 2)
