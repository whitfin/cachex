defmodule Cachex.Services.Informant do
  ...
  """
  def start_link(%State{ pre_hooks: [], post_hooks: [] }),
    do: :ignore
  def start_link(%State{ pre_hooks: pre_hooks, post_hooks: post_hooks }) do
    pre_hooks
  ...
  """
  def broadcast(%State{ pre_hooks: pre_hooks }, action),
    do: notify(pre_hooks, action, nil)

  @doc """
  Broadcasts an action and result to all post-hooks in a cache.
  """
  def broadcast(%State{ post_hooks: post_hooks }, action, result),
  ...
  """
  def link(%State{ pre_hooks: [], post_hooks: [] } = state),
    do: { :ok, state }
  def link(%State{ cache: cache, pre_hooks: pre_hooks, post_hooks: post_hooks } = state) do
    children =
      cache
